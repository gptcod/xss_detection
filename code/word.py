...
def save_len(normal,xss):
    with open("./file/len",'w') as f:
            f.write(str(normal.f_len+xss.f_len))
            
def maxlen(normal,xss):
    max=0
    if normal.max_len<xss.max_len:
        max=xss.max_len
    else:
        max=normal.max_len
    return max
